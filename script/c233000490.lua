...
function s.initial_effect(c)
	...
	c:RegisterEffect(e3)
	--cannot be destroyed by battle (full arch)
	local e4=Effect.CreateEffect(c)
	e4:SetType(EFFECT_TYPE_FIELD)
	e4:SetCode(EFFECT_INDESTRUCTABLE_BATTLE)
	e4:SetRange(LOCATION_GRAVE)
	e4:SetTargetRange(LOCATION_MZONE,0)
	e4:SetTarget(aux.TargetBoolFunction(Card.IsSetCard,0x7e8))
	e4:SetValue(1)
	c:RegisterEffect(e4)
end

function s.synval(e,c,sc)
	if sc:IsSetCard(0x7e8) and --c:IsNotTuner() 
		(not c:IsType(TYPE_TUNER) or c:IsHasEffect(EFFECT_NONTUNER)) and c:IsSetCard(0x7e8) and c:IsLocation(LOCATION_HAND) then
...
function s.filter(c)
	return c:IsSetCard(0x7e8) and c:GetCode()~=id and c:IsMonster() and c:IsAbleToHand()
end
