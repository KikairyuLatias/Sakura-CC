...
function s.condition(e,tp,eg,ep,ev,re,r,rp)
	return Duel.GetCurrentChain(true)==0 and eg:IsExists(Card.IsSummonPlayer,1,nil,1-tp)
		and Duel.IsExistingMatchingCard(aux.FaceupFilter(Card.IsSetCard,0x7e8),tp,LOCATION_MZONE,0,1,nil)
end
...
function s.actfilter(c)
	return c:IsFaceup() and c:IsSetCard(0x7e8) and c:IsAbleToExtra()
end
function s.actcon(e)
