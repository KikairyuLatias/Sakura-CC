...
function s.initial_effect(c)
	--link summon
	Link.AddProcedure(c,aux.FilterBoolFunction(Card.IsSetCard,0x439),3)
	--special summon
...
function s.spfilter(c,e,tp)
	return c:IsSetCard(0x439) and c:IsCanBeSpecialSummoned(e,0,tp,false,false,POS_FACEUP) and (c:IsLocation(LOCATION_HAND) or c:IsFaceup())
end
...
function s.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.SelectMatchingCard(tp,aux.NecroValleyFilter(s.spfilter),tp,LOCATION_EXTRA+LOCATION_HAND,0,1,1,nil,e,tp)
	local tc=g:GetFirst()
