...
function s.initial_effect(c)
	...
	c:RegisterEffect(e1)
	--Special summon 1 "Skateboard Dragon" monster
	...
	s.ritual_matching_function[c]=aux.FilterEqualFunction(Card.IsSetCard,0x5f0)
end

--the ritual stuff
s.listed_series={0x5f0}
function s.spfilter(c,e,tp,mc)
	return c:IsSetCard(0x5f0) and c:IsRitualMonster() and (not c.ritual_custom_check or c.ritual_custom_check(e,tp,Group.FromCards(mc),c))
...
function s.filter(c,e,tp)
	local sg=Duel.GetMatchingGroup(s.spfilter,tp,LOCATION_HAND+LOCATION_GRAVE,0,c,e,tp,c)
...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	local sg=Duel.GetMatchingGroup(s.spfilter,tp,LOCATION_HAND+LOCATION_GRAVE,0,mc,e,tp,mc)
...
function s.spfilter2(c,e,tp)
	return c:IsSetCard(0x5f0) and c:IsCanBeSpecialSummoned(e,0,tp,false,false,POS_FACEUP)
end
