--Rank-Up-Magic Techzodiac Force
local s,id=GetID()
function s.initial_effect(c)
	...
	e2:SetCode(EVENT_PHASE+PHASE_MAIN1+PHASE_MAIN2)
...
function s.filter1(c,e,tp)
	...
	return (#pg<=0 or (#pg==1 and pg:IsContains(c))) and c:IsFaceup() and c:IsSetCard(0x7d9) and (rk>0 or c:IsStatus(STATUS_NO_LEVEL))
...
function s.filter2(c,e,tp,mc,rk,pg)
	...
	return mc:IsType(TYPE_XYZ,c,SUMMON_TYPE_XYZ,tp) and c:IsRank(rk) and c:IsSetCard(0x7d9) and mc:IsCanBeXyzMaterial(c,tp)
