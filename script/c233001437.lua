--Sapphire☆Dream Honor Guard Command Rider Sakura
local s,id=GetID()
function s.initial_effect(c)
	--materials
	Synchro.AddProcedure(c,nil,1,1,aux.FilterSummonCode(233001427),1,1)
	c:EnableReviveLimit()
	--disable
...
function s.kamuitg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(Card.IsAbleToRemove,tp,0,LOCATION_HAND+LOCATION_ONFIELD+LOCATION_GRAVE,1,1) end 
...
function s.kamui(e,tp,eg,ep,ev,re,r,rp)
	...
	local tc=Duel.SelectMatchingCard(tp,Card.IsAbleToRemove,tp,0,LOCATION_HAND+LOCATION_ONFIELD+LOCATION_GRAVE,1,1,nil)
...
function s.mtop(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.GetDecktopGroup(tp,5):Filter(s.mtfilter,nil,e,tp)
