--Rudolph the Champion Starblossom Reindeer Athlete
local s,id=GetID()
...
--search for Starblossom stuff
function s.filter1(c)
	return c:IsSetCard(0x4cb) and c:IsAbleToHand()
end
...
function s.cfilter(c,tp)
	return c:IsPreviousControler(tp) and c:IsPreviousLocation(LOCATION_MZONE) and c:IsSetCard(0x4cb) and not c:IsCode(id)
end
...
s.listed_series={0x4cb}
function s.cfilter(c,tp)
	return c:IsFaceup() and c:IsSetCard(0x4cb) and c:IsControler(tp)
end
function s.actcon(e)
...
function s.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	local tc=nil
	if a:GetControler()==p and a:IsSetCard(0x4cb) then tc=d
	elseif d:GetControler()==p and d:IsSetCard(0x4cb) then tc=a end
