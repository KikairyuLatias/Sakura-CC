--Starblossom Reindeer Dream Stadium
local s,id=GetID()
function s.initial_effect(c)
	...
	e2:SetTarget(aux.TargetBoolFunction(Card.IsSetCard,0x4cb))
	e2:SetValue(500)
	c:RegisterEffect(e2)
	--def boost
	local e3=e2:Clone()
...
function s.spfilter(c,e,tp)
	return c:IsSetCard(0x4cb) and c:IsRace(RACE_BEASTWARRIOR) and c:IsCanBeSpecialSummoned(e,0,tp,false,false,POS_FACEUP)
end
...
function s.limfilter(c,sp)
	return c:IsSetCard(0x4cb) and c:IsFaceup() and c:IsSummonPlayer(sp)
end
