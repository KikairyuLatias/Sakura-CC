--Dream★Star Thousand Blossom Dance
local s,id=GetID()
...
--condition 2 (if Sakura or Dreamlight exists)
function s.remcon(e)
	return Duel.GetMatchingGroupCount(s.cfilter2,e:GetHandler():GetControler(),LOCATION_MZONE,0,nil)>=1
end
function s.cfilter2(c)
	return c:IsFaceup() and c:IsSetCard(0x5F6) and c:IsType(TYPE_MONSTER) and c:IsLevelAbove(7)
end
